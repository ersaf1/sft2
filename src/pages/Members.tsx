...
const Members = () => {
    ...
    const members: Member[] = [
        ...
        { id: 11, name: "ERSAF SYIRAZI ARIFIN", instagram: "3rsapp", role: "Anggota g guna", photo: "/members/11.png" },
        ...
        <div className="join w-full rounded-lg overflow-hidden shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 relative z-10">
                {filteredMembers.map((member, idx) => (
                    <div
                        key={member.id}
                        className="card card-animated-bg text-white shadow-xl transform-gpu hover:scale-[1.02] hover:shadow-2xl transition-all duration-300 ease-out animate-fade-in group"
                        style={{ animationDelay: `${idx * 80}ms` }}
                    >
                        <div className="card-body p-6">
                            <div className="flex justify-center mb-5">
                                <div className="avatar">
                                    <div className="w-24 rounded-full ring-2 ring-white/15 ring-offset-2 ring-offset-transparent group-hover:ring-white/30 transition-all duration-300">
                                        {member.photo ? (
                                            <img 
                                                src={member.photo} 
                                                alt={member.name}
                                        ...
                                        <div className={`bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm text-white rounded-full w-24 h-24 flex items-center justify-center ${member.photo ? 'hidden' : ''}`}>
                            ...
                            <div className="text-center space-y-2">
                                <h2 className="text-base font-bold leading-tight group-hover:text-white/95 transition-colors">
                                    {member.name}
                                </h2>
                                ...
                                {member.instagram && (
                                    <a 
                                        ...
                                        className="inline-flex items-center justify-center text-white/70 hover:text-white hover:scale-110 transition-all duration-200"
                                    >
